...
DROP DATABASE IF EXISTS saleswarehouse;
...
CREATE TABLE salesWarehouse.stagingDWH LIKE sales.Orders;

INSERT INTO salesWarehouse.stagingDWH SELECT * FROM sales.orders;
SELECT * FROM saleswarehouse.stagingDWH;

-- APPLY TRANSFORMATIONS

SELECT * FROM saleswarehouse.stagingdwh WHERE Quantity IS NOT NULL;

-- NOW WE HAVE TWO OPTIONS LIKE WE CAN DIRECTLY SHARE THE DATA TO OUR CORE LAYER OR WE CAN CREATE A TABLE OR VIEW(CREATING ANOTHER LAYER OR VIEW)

CREATE VIEW salesWarehouse.transform_sales AS SELECT * FROM saleswarehouse.stagingdwh WHERE Quantity IS NOT NULL;

SELECT * FROM saleswarehouse.transform_sales;

-- CORE LAYER

CREATE TABLE saleswarehouse.core_sales AS
SELECT * FROM saleswarehouse.transform_sales;

SELECT * FROM saleswarehouse.core_sales;

-- INCREMENTAL LOADING 

INSERT INTO sales.Orders (OrderID, OrderDate, CustomerID, CustomerName, CustomerEmail, ProductID, ProductName, ProductCategory,
RegionID, RegionName, Country, Quantity, UnitPrice, TotalAmount)
VALUES
...
INSERT INTO saleswarehouse.stagingdwh
SELECT *
FROM sales.orders
WHERE OrderDate > (SELECT MAX(OrderDate) FROM saleswarehouse.stagingdwh);
...
INSERT INTO saleswarehouse.core_sales 
SELECT * FROM saleswarehouse.transform_sales
WHERE orderDate > (SELECT MAX(orderDate) FROM saleswarehouse.core_sales);
